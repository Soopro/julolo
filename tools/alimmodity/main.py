    ...
    for item in sheetdata:
        ...
        try:
            output_data.append({
                ...
                'volume': int(item.get(u'商品月销量', -1)),
                'income_rate': float(item.get(u'收入比率', -1)),
                'commission': float(item.get(u'佣金', -1)),
                ...
                'coupon_volume': int(item.get(u'优惠券总量', -1)),
                'coupon_remine': int(item.get(u'优惠券剩余量', -1)),
