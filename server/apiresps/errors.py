...
class APIError(Exception):
    ...
    def __str__(self):
        if self.message:
            return '{}: {}'.format(self.status_message, self.message)
        else:
...
class RequestMaxLimited(MethodNotAllowed):
    response_code = 103010
    status_message = 'REQUEST_REACHES_MAX_LIMIT'


class RequestBlocked(MethodNotAllowed):
    response_code = 103020
    status_message = 'REQUEST_BLOCKED'


class Incapability(MethodNotAllowed):
    response_code = 103030
    status_message = 'INCAPABILITY'
